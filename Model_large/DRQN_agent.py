...
class drqn_agent_efficient():
    def __init__(self,N_station,h_size,lstm_units,tau,sess,batch_size,train_length,is_gpu=0,ckpt_path=None):
        ...
        self.eta = 0.5
        self.quantile_mask = self.mask ** self.eta / (self.mask ** self.eta + (1 - self.mask) ** self.eta) ** (
                    1 / self.eta)
        ...
        self.scalarInput = tf.placeholder(shape=[None, N_station * N_station * 4], dtype=tf.float32,name='main_input')

        self.imageIn = tf.reshape(self.scalarInput, shape=[-1, N_station, N_station, 4])
    ...
    def build_train(self):
        ...
        for i in range(self.N_station):
            ...
            loss = self._compute_loss(tf.sort(mainz,axis=-1),tf.sort(self.targetQ[i],axis=-1))
    ...
    def _compute_loss(self, mainQ, targetQ):
        ...
        quantile_loss = tf.reduce_sum(quantile_loss*self.quantile_mask, axis=-1)  # Expected loss for each quantile
