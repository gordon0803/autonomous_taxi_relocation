...
class drqn_agent_efficient():
    ...
    def build_main(self):
        ...
        imageIn = tf.reshape(self.scalarInput[0], shape=[-1, self.N_station, self.N_station, 5], name=myScope_main + 'in1')
        ...
        for i in range(self.N_station):
                ...
                conv1 = tf.layers.conv2d( \
                    inputs=input_conv, filters=16, \
                    kernel_size=[5, 5], strides=[2, 2], activation='relu', reuse=True, padding='VALID', \
                    name=myScope_main + '_net_conv1')
                conv2 = tf.layers.conv2d( \
                    inputs=conv1, filters=32, \
                    kernel_size=[3, 3], strides=[1, 1], activation='relu', reuse=True, padding='VALID', \
                    name=myScope_main + '_net_conv2')
                ...
                Advantage=tf.layers.dense(streamA,(self.N_station) * self.N,name=myScope+'AW',reuse=None) #advantage
                ...
                Advantage2=tf.layers.dense(streamA2,(self.N_station) * self.N,name=myScope+'AW',reuse=True) #advantage
                Value2=tf.layers.dense(streamV2,1,name=myScope+'VW',reuse=True) #advantage
                Qt2 = Value2 + tf.subtract(Advantage2, tf.reduce_mean(Advantage2, axis=1, keepdims=True),name=myScope+'_unshaped_Qout')
